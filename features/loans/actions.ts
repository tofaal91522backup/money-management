...
function refreshLoans() { revalidatePath("/"); revalidatePath("/accounts"); revalidatePath("/loans") }
...
export async function createRepaymentAction(_: LoanFormState, formData: FormData): Promise<LoanFormState> {
  const user = await requireUser(); const loanId = String(formData.get("loanId") ?? ""); const accountId = String(formData.get("accountId") ?? ""); const amount = parseMoneyInput(String(formData.get("amount") ?? "")); const date = parseDate(String(formData.get("date") ?? "")); const note = String(formData.get("note") ?? "").trim()
  ...
  if (amount > remaining) return { error: "Repayment cannot be larger than the remaining amount." }
  const account = await prisma.account.findFirst({ where: { id: accountId, userId: user.id, isArchived: false }, select: { id: true } }); if (!account) return { error: "Choose an active account." }
  ...
  await prisma.$transaction(async (tx) => { const repayment = await tx.loanRepayment.create({ data: { amount, date, note: note || null, userId: user.id, loanId: loan.id, accountId: account.id } }); await tx.transaction.create({ data: { type: "LOAN_REPAYMENT", amount, date, note: note || null, userId: user.id, loanId: loan.id, repaymentId: repayment.id, ...(loan.type === "RECEIVABLE" ? { destinationAccountId: account.id } : { sourceAccountId: account.id }) } }); const totalRepaid = repaid + amount; await tx.loan.update({ where: { id: loan.id }, data: { status: totalRepaid === loan.originalAmount ? "PAID" : "PARTIALLY_PAID" } }) })
  refreshLoans(); return { success: true }
}
