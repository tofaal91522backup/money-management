...
export async function GET() {
  ...
  const loans = await prisma.loan.findMany({ where: { userId: user.id }, orderBy: [{ status: "asc" }, { dueDate: "asc" }, { createdAt: "desc" }], include: { originAccount: { select: { id: true, name: true } }, transactions: { where: { type: "LOAN_DISBURSEMENT" }, select: { id: true } }, repayments: { orderBy: { date: "desc" }, include: { account: { select: { id: true, name: true } } } } } })
